...
export type Reason = 'start_anchor' | 'outside_corridor' | 'target_miss' | 'over_budget' | null;
...
export function validate(rules: Rules, raw: readonly Point[]): Validation {
  ...
  const reason: Reason = !startsAtAnchor ? 'start_anchor' : !staysInCorridor ? 'outside_corridor' : !endsAtTarget ? 'target_miss' : length > rules.budget ? 'over_budget' : null;
