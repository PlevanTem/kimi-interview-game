 ...
 * 八枚章在图版上的落点。
 *
 * 排成一条**缓 S**，不是一条直线也不是一张地理地图——
 * 八座岛在世界里彼此毫无关系（`scenes/index.ts` 写明了这一点），
 * 图上如果给出方位与距离，就等于凭空发明了一片地中海。
 * 这条 S 只表示先后，不表示"在哪儿"。
 *
 * z 越负越远：航程往里退进雾里，未到的幕天然就在最深处。
 */
const SLOTS: readonly ChartSlot[] = [
  { x: -3.55, z: 0.95, yaw: 0.22 },
  { x: -2.5, z: -0.05, yaw: -0.5 },
  { x: -1.35, z: 0.5, yaw: 0.34 },
  { x: -0.2, z: -0.35, yaw: -0.18 },
  { x: 0.95, z: 0.42, yaw: 0.62 },
  { x: 2.05, z: -0.42, yaw: -0.72 },
  { x: 3.1, z: 0.3, yaw: 0.1 },
  { x: 4.15, z: -0.62, yaw: -0.28 },
];
